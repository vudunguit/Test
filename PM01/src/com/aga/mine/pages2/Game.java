...
import com.aga.mine.pages2.GameEnding;
...
public class Game extends CCLayer implements MineCell.MineCellDelegate {
	...
	Game() {
		...
		for (int i = 0; i < size; i++) {
			if (cellsTemp.get(i).isOpened()) {
				...
				if (numberOfMine > 0)
					this.displayMineNumber(numberOfMine, cellsTemp.get(i)
									...
									"unopenedTile : "
											...
											+ GameData
													...
													.getMaxMineNumber(
															GameData.share()
																	.getGameDifficulty()));
							if (unopenedTile == GameData.share()
									.getMaxMineNumber(
											GameData.share()
													.getGameDifficulty())) {
								Log.e("Game / handleLongPress",
										"handleLongPress Game Over");
								this.gameOver();
								// Log.e("Game / handleLongPress", "mine:" +
								// mine + ", difficulty : " +
	...
	public void handleDoubleTap(MotionEvent event) {
		Log.e("Game / handleDoubleTap", "마인 갯수 : "
				+ GameData.share().getMineNumber());
...
//						}).start();
			...
			Log.e("Game / handleDoubleTap",
					"unopenedTile : "
							+ unopenedTile
							+ ", Max Mine : "
							+ GameData.share().getMaxMineNumber(
									GameData.share().getGameDifficulty()));

			if (unopenedTile == GameData.share().getMaxMineNumber(
					GameData.share().getGameDifficulty())) {
	...
	public void displayMineNumber(int numberOfMine, CGPoint position, int tag) {
		...
		CCLabel label = CCLabel.makeLabel("" + numberOfMine + " ",
				"Arial-Bold", (int) ((70 * (2 / 3.0) * tileSize.width) / 128));
	...
	public static class HudLayer extends CCLayer {
		...
		public HudLayer() {
			...
			this.addChild(GameMinimap.getInstance(),
